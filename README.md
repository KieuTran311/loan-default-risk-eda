...
- Checked for weird values, found a handful of rows with age over 100 (one is 144) and a couple with 123 years of employment length. I didn't just delete these, I only checked how many rows would be affected (7 rows out of 32k) and it barely moves the overall default rate either way, so I left them in and just noted it.
...
- I didn't remove the handful of unrealistic age/employment values, just flagged them, so there's a small amount of noise in the data.
...
Overall default rate in this dataset was 21.9% and it varied a lot more by LTI (11.6% up to 74.5%) than by loan purpose (14.8% to 28.7%). That LTI pattern held up across almost every loan intent when I cross-tabbed them, which is the strongest single pattern I found here, though it's still just EDA on one dataset, not a model.
